<?php
/**
 * MLib_Media_Image_Metabox: Image metabox for mlib-media-meta-img
 *
 * @package Movie_Library
 */

namespace Movie_Library\Inc\Meta_Boxes\Photo;

use WP_Post;
use Movie_Library\Inc\Meta_Boxes\Base;

/**
 * MLib_Media_Image_Metabox for metabox on Image.
 */
class MLib_Media_Image_Metabox extends Base {
	/**
	 * Metabox ID
	 *
	 * @since 0.1.0
	 */
	protected const ID = 'mlib-media-meta-img';
	 ...
	 * @since 0.1.0
	 */
	public static $attachment_id_key = 'mlib-media-meta-img';
	...
	public function render_meta_box( $post ) {
		...
		$movie_library_image_gallery = json_decode(
			get_post_meta(
				$post->ID,
		...
		?>
		<button id="movie-library_add_image_btn"><?php esc_html_e( 'Add Image', 'movie-library' ); ?></button>
		<div id="movie-library_image_container">
			<ul class="movie-library_images">
				<?php
					$attachments = $movie_library_image_gallery ?? array();
			...
			<input
				type="hidden"
				id="movie_library_image_gallery"
				name="movie_library_image_gallery"
				value="<?php echo esc_attr( implode( ',', $movie_library_image_gallery ) ); ?>"
			/>
		...
		$script_data       = <<<SCRIPT_DATA
				const mlbImageGalleryTranslatedStrings = {
	...
	public function save_metadata( $post_id ) {
		...
		if ( true === isset( $_POST['movie_library_image_gallery'] ) ) {
			$attachments = sanitize_text_field(
				wp_unslash(
					$_POST['movie_library_image_gallery']
				)
			);
