...
namespace Movie_Library\Inc\Helpers;
...
final class AutoLoader {
	...
	/**
	 * Parse the path of the class given the relative class name.
	 *
	 * @param string $relative_class_name The relative class name.
	 * @return string
	 */
	private function parse_class_path( string $relative_class_name ): string {
		...
		$file_name = sprintf( 'class-%s', $file_name );
	...
	private function load_mapped_file(
		...
		foreach (
			...
			$this->parse_class_path( $relative_class_name ) .
			'.php';
	 ...
	 * @return string|bool Boolean false if the class file is not found, the path to the mapped file on success.
	 */
